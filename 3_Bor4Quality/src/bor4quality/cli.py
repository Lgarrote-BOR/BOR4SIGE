...
def _run(path, *, snapshot_id="pilot-quality-2026-06-21"):
    ...
    audit_scenarios = [
        {"desc": "Cliente firmando su QOE (capacidad propia)", "acc": "oe.firmar", "rol": RolModulo.CLIENTE.value, "roles_modulo": [RolModulo.CLIENTE.value], "expected": "ALLOW"},
        {"desc": "Revisor intentando publicar catalogo (sin capacidad)", "acc": "catalogo.publicar", "rol": RolModulo.REVISOR.value, "roles_modulo": [RolModulo.REVISOR.value], "expected": "DENY"},
        {"desc": "Mantenedor + Revisor firmando QOE (SoD estricto)", "acc": "oe.firmar", "rol": RolModulo.MANTENEDOR.value, "roles_modulo": [RolModulo.MANTENEDOR.value, RolModulo.REVISOR.value], "expected": "DENY"},
        {"desc": "Mantenedor publicando catalogo (capacidad propia)", "acc": "catalogo.publicar", "rol": RolModulo.MANTENEDOR.value, "roles_modulo": [RolModulo.MANTENEDOR.value], "expected": "ALLOW"},
        {"desc": "Cliente intentando dictaminar QOE (sin capacidad)", "acc": "oe.dictaminar", "rol": RolModulo.CLIENTE.value, "roles_modulo": [RolModulo.CLIENTE.value], "expected": "DENY"},
    ]
    rbac_audit = []
    ...
    return {
        ...
        "oes": oes,
        "rbac_check_ejemplo": {"scope": "oe.firmar", "permit": alcance_qoe.permit, "outcome": alcance_qoe.outcome},
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        prog="bor4quality-cli",
        description="Clon Quality ISO 9001 - genera QOE firmables a partir de un perfil.",
    )
    ...
    parser.add_argument(
        "--snapshot-id",
        default="pilot-quality-2026-06-21",
