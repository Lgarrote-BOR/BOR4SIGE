...
<style>
/* ── RESET SCOPE ── */
.sige20-wp *, .sige20-wp *::before, .sige20-wp *::after {
...
.sige20-wp .s-btn-p:hover{transform:translateY(-2px);box-shadow:0 0 50px rgba(59,130,246,.6);}
.sige20-wp .s-btn-s {
  display:inline-flex;align-items:center;gap:.5rem;
...
.sige20-wp .s-btn-s:hover{background:rgba(255,255,255,.1);transform:translateY(-2px);}
.sige20-wp .s-hero-stats {
  display:flex;gap:3rem;margin-top:4rem;
  animation:sige-fadeUp .6s .4s ease both;
  flex-wrap:wrap;justify-content:center;
}
.sige20-wp .s-stat-num {
  font-family:var(--font-head);font-size:2.25rem;font-weight:800;
  background:var(--gradient-accent);
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;
}
.sige20-wp .s-stat-label{font-size:.8rem;color:var(--c-muted);font-weight:500;margin-top:.25rem;}

/* ── NORMAS STRIP ── */
.sige20-wp .s-normas-strip {
  background:var(--c-surface);
  border-top:1px solid var(--c-border);border-bottom:1px solid var(--c-border);
  overflow:hidden;padding:1.25rem 0;
}
.sige20-wp .s-normas-track {
  display:flex;gap:3rem;
  animation:sige-marquee 25s linear infinite;white-space:nowrap;
}
.sige20-wp .s-norma-item {display:flex;align-items:center;gap:.75rem;flex-shrink:0;}
.sige20-wp .s-norma-icon {
  width:36px;height:36px;border-radius:8px;
  display:flex;align-items:center;justify-content:center;
  font-size:.65rem;font-weight:800;color:#fff;
}
.sige20-wp .s-norma-name {font-size:.85rem;font-weight:600;color:var(--c-muted);}

/* ── SECTIONS ── */
.sige20-wp .s-inner {max-width:1200px;margin:0 auto;padding:5rem 2rem;}
.sige20-wp .s-label {
  display:inline-flex;align-items:center;gap:.5rem;
  font-size:.75rem;font-weight:700;letter-spacing:2px;
  text-transform:uppercase;color:#60a5fa;margin-bottom:1rem;
}
.sige20-wp .s-label::before{content:'';width:20px;height:2px;background:#60a5fa;}
.sige20-wp .s-title {
  font-family:var(--font-head);
  font-size:clamp(1.8rem,3.5vw,2.8rem);
  font-weight:800;letter-spacing:-1px;line-height:1.1;
}
.sige20-wp .s-desc{color:var(--c-muted);font-size:1.05rem;max-width:600px;margin-top:1rem;line-height:1.7;}

/* ── FEATURES ── */
.sige20-wp .s-features-hdr{text-align:center;margin-bottom:3rem;}
.sige20-wp .s-features-grid {
...
.sige20-wp .s-fcard::before{
  content:'';position:absolute;top:0;left:0;right:0;height:1px;
  background:linear-gradient(90deg,transparent,rgba(255,255,255,.1),transparent);
}
.sige20-wp .s-fcard:hover{transform:translateY(-4px);border-color:rgba(59,130,246,.3);box-shadow:0 20px 60px rgba(59,130,246,.1);}
.sige20-wp .s-fcard-icon{width:52px;height:52px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:1.5rem;margin-bottom:1.25rem;}
.sige20-wp .s-fcard-title{font-family:var(--font-head);font-size:1.1rem;font-weight:700;margin-bottom:.6rem;}
.sige20-wp .s-fcard-desc{font-size:.9rem;color:var(--c-muted);line-height:1.65;}
.sige20-wp .s-ftags{display:flex;flex-wrap:wrap;gap:.4rem;margin-top:1.25rem;}
.sige20-wp .s-ftag{font-size:.7rem;font-weight:600;padding:.2rem .6rem;border-radius:100px;background:rgba(59,130,246,.1);color:#93c5fd;border:1px solid rgba(59,130,246,.2);}

/* ── STATS BAND ── */
.sige20-wp .s-stats-band{background:var(--c-surface);border-top:1px solid var(--c-border);border-bottom:1px solid var(--c-border);}
.sige20-wp .s-stats-grid{display:grid;grid-template-columns:repeat(4,1fr);}
.sige20-wp .s-stat-box{padding:3rem 2rem;text-align:center;border-right:1px solid var(--c-border);}
.sige20-wp .s-stat-box:last-child{border-right:none;}
.sige20-wp .s-snum{font-family:var(--font-head);font-size:3rem;font-weight:900;line-height:1;background:var(--gradient-accent);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.sige20-wp .s-slabel{font-size:.85rem;color:var(--c-muted);margin-top:.5rem;font-weight:500;}

/* ── MODULES ── */
.sige20-wp .s-mods-bg{background:var(--c-surface);border-top:1px solid var(--c-border);border-bottom:1px solid var(--c-border);}
.sige20-wp .s-mods-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:1rem;margin-top:2rem;}
.sige20-wp .s-mcard{background:var(--gradient-card);border:1px solid var(--c-border);border-radius:16px;padding:1.5rem;transition:transform .2s,box-shadow .2s;}
.sige20-wp .s-mcard:hover{transform:translateY(-3px);box-shadow:0 12px 40px rgba(0,0,0,.3);}
.sige20-wp .s-mcard-icon{font-size:1.75rem;margin-bottom:.75rem;}
.sige20-wp .s-mcard-title{font-size:.85rem;font-weight:700;margin-bottom:.25rem;}
.sige20-wp .s-mcard-norm{font-size:.7rem;color:var(--c-muted);}

/* ── AI SECTION ── */
.sige20-wp .s-ai-bg{background:linear-gradient(135deg,#0a0e1a 0%,#0d1120 50%,#070d1f 100%);}
.sige20-wp .s-ai-layout{display:grid;grid-template-columns:1fr 1fr;gap:4rem;align-items:center;}
.sige20-wp .s-aicard{background:var(--gradient-card);border:1px solid var(--c-border);border-radius:20px;padding:2rem;position:relative;overflow:hidden;}
.sige20-wp .s-aicard::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(59,130,246,.05),rgba(139,92,246,.05));}
.sige20-wp .s-scan{position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#3b82f6,transparent);animation:sige-scan 3s ease-in-out infinite;}
.sige20-wp .s-alert{display:flex;align-items:flex-start;gap:.75rem;padding:.9rem 1rem;border-radius:10px;margin-bottom:.75rem;font-size:.82rem;}
.sige20-wp .s-alert-ok{background:rgba(16,185,129,.08);border:1px solid rgba(16,185,129,.2);}
.sige20-wp .s-alert-warn{background:rgba(245,158,11,.08);border:1px solid rgba(245,158,11,.2);}
.sige20-wp .s-alert-info{background:rgba(59,130,246,.08);border:1px solid rgba(59,130,246,.2);}
.sige20-wp .s-adot{width:8px;height:8px;border-radius:50%;margin-top:3px;flex-shrink:0;}
.sige20-wp .s-ai-ul{list-style:none;margin-top:1.5rem;}
.sige20-wp .s-ai-ul li{display:flex;align-items:flex-start;gap:.75rem;padding:.75rem 0;border-bottom:1px solid var(--c-border);font-size:.9rem;}
.sige20-wp .s-ai-ul li:last-child{border-bottom:none;}

/* ── STANDARDS ── */
.sige20-wp .s-std-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:1rem;margin-top:3rem;}
.sige20-wp .s-std-card{background:var(--gradient-card);border:1px solid var(--c-border);border-radius:16px;padding:1.5rem;text-align:center;transition:transform .2s,border-color .2s;position:relative;overflow:hidden;}
.sige20-wp .s-std-card:hover{transform:translateY(-3px);border-color:rgba(139,92,246,.4);}
.sige20-wp .s-std-glow{position:absolute;top:-20px;right:-20px;width:80px;height:80px;border-radius:50%;filter:blur(30px);opacity:.2;}
.sige20-wp .s-std-badge{display:inline-block;font-family:var(--font-head);font-size:.7rem;font-weight:800;padding:.25rem .6rem;border-radius:6px;margin-bottom:.75rem;color:#fff;}
.sige20-wp .s-std-name{font-size:1.05rem;font-weight:700;margin-bottom:.25rem;}
.sige20-wp .s-std-desc{font-size:.75rem;color:var(--c-muted);line-height:1.5;}
.sige20-wp .s-std-bar{margin-top:1rem;height:4px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden;}
.sige20-wp .s-std-fill{height:100%;border-radius:2px;}
.sige20-wp .s-std-pct{font-size:.7rem;color:var(--c-muted);margin-top:.35rem;text-align:right;}

/* ── CHATBOT DEMO ── */
.sige20-wp .s-chat-layout{display:grid;grid-template-columns:1fr 1fr;gap:4rem;align-items:center;}
.sige20-wp .s-chat-bg{background:var(--c-surface);border-top:1px solid var(--c-border);}
.sige20-wp .s-chat-win{background:var(--gradient-card);border:1px solid var(--c-border);border-radius:20px;overflow:hidden;}
.sige20-wp .s-chat-hdr{display:flex;align-items:center;gap:.75rem;padding:1rem 1.25rem;background:rgba(255,255,255,.03);border-bottom:1px solid var(--c-border);}
.sige20-wp .s-chat-av{width:32px;height:32px;background:var(--gradient-accent);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.9rem;}
.sige20-wp .s-chat-name{font-size:.85rem;font-weight:700;}
.sige20-wp .s-chat-status{font-size:.7rem;color:#10b981;}
.sige20-wp .s-chat-body{padding:1.25rem;display:flex;flex-direction:column;gap:1rem;}
.sige20-wp .s-cmsg{display:flex;gap:.75rem;align-items:flex-start;}
.sige20-wp .s-cmsg.user{flex-direction:row-reverse;}
.sige20-wp .s-cbubble{max-width:75%;padding:.75rem 1rem;border-radius:14px;font-size:.82rem;line-height:1.5;}
.sige20-wp .s-cbot{background:rgba(59,130,246,.1);border:1px solid rgba(59,130,246,.2);border-radius:4px 14px 14px 14px;}
.sige20-wp .s-cuser{background:var(--gradient-accent);border-radius:14px 14px 4px 14px;color:#fff;}
.sige20-wp .s-chat-in{display:flex;align-items:center;gap:.75rem;padding:1rem 1.25rem;background:rgba(255,255,255,.02);border-top:1px solid var(--c-border);}
.sige20-wp .s-chat-field{flex:1;background:rgba(255,255,255,.04);border:1px solid var(--c-border);border-radius:10px;padding:.6rem 1rem;color:var(--c-text);font-size:.82rem;outline:none;}
.sige20-wp .s-chat-send{width:34px;height:34px;background:var(--gradient-accent);border:none;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;font-size:.85rem;}

/* ── CTA ── */
.sige20-wp .s-cta{background:linear-gradient(135deg,#0f172a 0%,#1e1b4b 50%,#0f172a 100%);text-align:center;}
.sige20-wp .s-cta-box{max-width:700px;margin:0 auto;}
.sige20-wp .s-cta-box h2{font-family:var(--font-head);font-size:clamp(1.8rem,3.5vw,2.8rem);font-weight:800;letter-spacing:-1px;line-height:1.1;margin-bottom:1rem;}
.sige20-wp .s-cta-box p{color:var(--c-muted);font-size:1.05rem;line-height:1.7;margin-bottom:2rem;}
.sige20-wp .s-perks{display:flex;gap:2rem;margin-top:3rem;justify-content:center;flex-wrap:wrap;}
.sige20-wp .s-perk{display:flex;align-items:center;gap:.5rem;font-size:.85rem;color:var(--c-muted);}

/* ── SCROLL REVEAL ── */
.sige20-wp .s-reveal{opacity:0;transform:translateY(30px);transition:opacity .7s ease,transform .7s ease;}
.sige20-wp .s-reveal.visible{opacity:1;transform:translateY(0);}

/* ── RESPONSIVE ── */
@media(max-width:900px){
  .sige20-wp .s-ai-layout,.sige20-wp .s-chat-layout{grid-template-columns:1fr;}
  .sige20-wp .s-stats-grid{grid-template-columns:1fr 1fr;}
  .sige20-wp .s-stat-box{border-right:1px solid var(--c-border);border-bottom:1px solid var(--c-border);}
}
@media(max-width:600px){
  .sige20-wp .s-features-grid{grid-template-columns:1fr;}
  .sige20-wp .s-stats-grid{grid-template-columns:1fr 1fr;}
}
</style>

<!-- ═══════════════════════════════════════════════
     CONTENIDO — SIGE 2.0 LANDING PAGE (WordPress)
     ═══════════════════════════════════════════════ -->
<div class="sige20-wp">
  ...
  <section class="s-hero">
    ...
    <p class="s-hero-sub">
      <strong>SIGE 2.0</strong> centraliza el cumplimiento de las principales normas ISO y el Esquema Nacional de Seguridad en una sola plataforma, potenciada con Inteligencia Artificial para auditorías automáticas y seguimiento continuo.
    </p>
  ...
  <section id="funcionalidades-sige">
    <div class="s-inner">
      <div class="s-features-hdr s-reveal">
        ...
        <p class="s-desc" style="margin:0 auto">SIGE 2.0 integra gestión de calidad, seguridad, medio ambiente, compliance y servicios TI en un entorno unificado, intuitivo y potente.</p>
      </div>
  ...
  <section>
    <div class="s-inner">
      <div class="s-reveal" style="text-align:center;margin-bottom:1rem">
        ...
        <p class="s-desc" style="margin:1rem auto">SIGE 2.0 cubre el ciclo de vida completo de cumplimiento para las principales normas internacionales y el esquema español de seguridad.</p>
      </div>
      ...
      <div class="s-chat-layout">
        ...
        <div class="s-chat-win s-reveal">
          <div class="s-chat-hdr">
            ...
            <div><div class="s-chat-name">Asistente SIGE 2.0</div><div class="s-chat-status">● En línea</div></div>
          </div>
          <div class="s-chat-body">
            <div class="s-cmsg"><div class="s-cbubble s-cbot">¡Hola! Soy tu asistente de SIGE 2.0. Puedo ayudarte con los módulos o explicarte requisitos de las normas ISO y ENS. ¿En qué puedo ayudarte?</div></div>
  ...
  <section class="s-cta" id="contacto-sige">
    <div class="s-inner">
      <div class="s-cta-box s-reveal">
        ...
        <p>SIGE 2.0 funciona tanto en red local como offline desde un pendrive. Sin dependencias de nube, sin suscripciones. Tu dato, tu control.</p>
        <div style="display:flex;gap:1rem;justify-content:center;flex-wrap:wrap;margin-top:0">
          <a href="#" class="s-btn-p">🚀 Solicitar acceso a SIGE 2.0</a>
...
(function(){
  /* ── SCROLL REVEAL ── */
  var revEls = document.querySelectorAll('.sige20-wp .s-reveal');
  ...
  var statsEl = document.querySelector('.sige20-wp .s-stats-band');
  if(statsEl){
